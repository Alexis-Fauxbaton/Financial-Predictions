...
class BinanceDataFetcher(DataFetcher):
    ...
    def __init__(self, pair = Pairs.BTCUSDT.value, timeframe=TimeFrame.M15.value, start_year = "2023", start_month = "01") -> None:
        super().__init__()
    ...
    def getAllCandlesUntil(self, end_date = "2023-02-28"):
        
        start_time = "2017-08-01"
                
        format = "%Y-%m-%d"
        
        start_timestamp = int(datetime.strptime(start_time, format).timestamp()) * 1000 #*1000 to convert to milliseconds
        ...
        curr_time = self.client.ui_klines("BTCUSDT", "15m", limit=1, startTime = start_timestamp)[0][0]
        ...
        while curr_time < end_timestamp:
            ...
            r = self.client.ui_klines("BTCUSDT", "15m", limit=limit, startTime = curr_time)
            candles += [r[i][:-3] for i in range(len(r))]

            curr_time += 15 * 60 * limit * 1000 #*1000 to convert to milliseconds
        
        print(str(dt_object))
        data = pd.DataFrame(candles, columns=["Unix", "Open", "High", "Low", "Close", "Volume", "Close Unix", "Quote Asset Volume", "NTrades"])
        print()
        print(data.tail())
        print(data.shape)
        ...
        data.to_csv(f"fetched_{self.pair}.csv")
        
if __name__ == "__main__":
    fetcher = BinanceDataFetcher()
    
    fetcher.getAllCandlesUntil()