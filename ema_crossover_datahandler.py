...
def standard_labeling(data, max_days, target_range, skip_factor=3):
    ...
    predict_data["Target"] = ((predict_data["EMA_50"].shift(-target_range) - predict_data["EMA_10"].shift(-target_range)) <= 0 ) == sign

    print("Value counts : ", predict_data["Target"].value_counts())
    
    predict_data["Target"] = np.where((predict_data["Target"] == False), 0, predict_data["Target"])    
    predict_data["Target"] = np.where((predict_data["Target"] == True) & (sign == True), -1, predict_data["Target"])
    predict_data["Target"] = np.where((predict_data["Target"] == True) & (sign == False), 1, predict_data["Target"])
    ...
    display_data = predict_data[["Unix", "Close", "EMA_10", "EMA_50", "Target"]][-100:].copy().reset_index(drop=True)
    predict_data.drop(
...
def simple_strategy_backtest(test_set):
    ...
    axis.plot(test_set["EMA_10"].index, test_set["EMA_10"], label="EMA_10")
    axis.plot(test_set["EMA_50"].index, test_set["EMA_50"], label="EMA_50")
    axis.scatter(test_set[test_set["Target"] == 0].index, test_set[test_set["Target"] == 0]["Close"], alpha = 0.5, color = 'r', marker = 'o', label="Sell")
    axis.scatter(test_set[test_set["Target"] == 2].index, test_set[test_set["Target"] == 2]["Close"], alpha = 0.5, color = 'b', marker = 'o', label="Buy")
...
class EMACrossoverDataHandler(DataHandler):
    ...
    #    model.fit(train_data, train_labels, epochs=epochs, validation_data=(test_data,test_labels),callbacks=[tensorboard_callback], batch_size=64)
        ...
        test_set["Close"] = self.data["Close"].loc[test_set.index]
        test_set["EMA_10"] = ta.ema(test_set["Close"], 10)
        test_set["EMA_50"] = ta.ema(test_set["Close"], 50)
